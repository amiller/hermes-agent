...
**Hermes wraps every outgoing chat request in an httpx transport that encrypts the conversation to a CVM-attested public key.** Only a TEE-verified GPU running the named model can decrypt it. This page is the *cryptographic-layer liveness* signal; for the *trust-model* gap analysis (ERC-733 stages, operator rug-risk), see [amiller/devproof-audits-guide](https://github.com/amiller/devproof-audits-guide).
...
A ✅ below means the live attestation bundle cryptographically checks out *at this run's timestamp*. It is **not** a claim that the end-to-end trust model is sound. Attestation says "this code is running in a TEE"; it does not answer "can the operator rug users?"

For the gap analysis, see **[amiller/devproof-audits-guide](https://github.com/amiller/devproof-audits-guide)** — an external audit repo tracking ERC-733 stages for TEE apps. Hermes is currently assessed there as **[Stage 0 / ruggable](https://github.com/amiller/devproof-audits-guide/tree/main/case-studies/hermes)**. The per-model ✅s on this page do not move that assessment until the items below are addressed:

- **Operator-controlled upgrade path.** Pha KMS has no public upgrade log, so "what code actually ran when a user sent their last message?" is not answerable after the fact.
- **Mutable image tags.** Compose files reference `image:tag` rather than `image@sha256:...`, so the running measurement can drift silently between pulls.
- **Configurable endpoints.** Base URLs and API keys flow in via env at runtime; a malicious operator can redirect traffic without breaking any on-box attestation.
- **No published notice period** for upgrades or key rotation — a Stage 1 DevProof requirement the current deployment does not meet.
- **Provider-side limits.** Some prover rows in the table fail (GPU attest, stale TCB, 503s) — those are per-run signals, not structural, but they also don't close the gaps above.

Closing these is tracked upstream in the audits-guide, not here. This page is the live "is the cryptographic layer healthy" signal; that page is "is the trust model actually Stage 1."
