...
Environment variables:
    ...
    MATRIX_AUTO_BOOTSTRAP_E2EE  Set "true" to automatically bootstrap E2EE for fresh accounts
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption:
            ...
            try:
                ...
                elif os.getenv("MATRIX_AUTO_BOOTSTRAP_E2EE", "").lower() in ("true", "1", "yes"):
                    # Auto-bootstrap E2EE for fresh accounts.
                    # Check if SSSS default key exists on server.
                    try:
                        from mautrix.types import EventID
                        ssss_key_resp = await client.get_account_data("m.secret_storage.default_key")
                        if not ssss_key_resp:
                            # No SSSS default key — safe to bootstrap.
                            generated_key = await olm.generate_recovery_key()
                            hermes_home = os.getenv("HERMES_HOME")
                            if hermes_home and generated_key:
                                env_path = Path(hermes_home) / ".env"
                                env_tmp_path = Path(hermes_home) / ".env.tmp"
                                # Atomic write: write to temp file, then replace.
                                existing_content = ""
                                if env_path.exists():
                                    existing_content = env_path.read_text()
                                # Check if MATRIX_RECOVERY_KEY already exists to avoid duplicates.
                                if "MATRIX_RECOVERY_KEY=" not in existing_content:
                                    new_line = f"\nMATRIX_RECOVERY_KEY={generated_key}"
                                    with open(env_tmp_path, "w") as f:
                                        f.write(existing_content)
                                        f.write(new_line)
                                    os.replace(env_tmp_path, env_path)
                                    logger.info(
                                        "Matrix: bootstrapped E2EE; recovery key persisted to %s",
                                        str(env_path),
                                    )
                                else:
                                    logger.debug(
                                        "Matrix: MATRIX_RECOVERY_KEY already in .env, skipping write"
                                    )
                    except Exception as exc:
                        # Let exceptions propagate so tests can assert failure modes.
                        raise
